document.addEventListener('DOMContentLoaded', function() {
    ...
    class HeartParticle {
        ...
        // 根据缩放比例计算颜色
        getColor(scale) {
            // 当scale最大时（1.15）是正红色，最小时（0.85）是浅粉色
            const red = 255;
            const green = Math.floor(175 - (scale - 0.85) * 200);
            const blue = Math.floor(175 - (scale - 0.85) * 200);
        ...
        draw(scale) {
            ...
            heartCtx.fillStyle = this.getColor(scale);
            heartCtx.fill();
        }
    ...
    function createHeartShape() {
        ...
        while (count < maxPoints) {
            ...
            if (isInHeart((x - cx) / 1.1 + cx, -(y - cy) + cy, cx, cy, scale)) {
                points.push({ x, y });
                count++;
            }
        }
